...
// stand-in -- it exposes a no-op fitToCoordinates/animateToRegion so
// native mapRef.current call sites stay safe on web.
const TrackingMap = forwardRef<
  { fitToCoordinates: () => void; animateToRegion: () => void },
  TrackingMapProps
>(({ style, isDriverOnline }, ref) => {
  ...
  return (
    <View style={[style, styles.placeholder]}>
      ...
      <Text style={styles.text}>
        {isDriverOnline
          ? 'Live driver tracking is available in the AabRahat mobile app.'
          : 'Waiting for driver — live tracking is available in the mobile app.'}
      </Text>
    </View>
  );
});
